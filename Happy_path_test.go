...
func TestHandleVoteMessageFromPeer(t *testing.T) {
	...
	node.votesSeen = votesSeen

	// Call handleVoteMessageFromPeer
	node.handleVoteMessageFromPeer(vote)

	// Assert that the vote was added to the voteseen map
	if _, ok := node.votesSeen.Votes[Hash(1, genesisBlock.Txns)][string(leaderPubKey)]; !ok {
	...
	voteStake := ComputeStake(node.votesSeen.Votes[Hash(1, genesisBlock.Txns)], node.PubKeyToStake)
...
func TestHandleBlockFromPeer(t *testing.T) {
	...
	node.handleBlockFromPeer(&block1)
	node.handleBlockFromPeer(&block2)
	node.handleBlockFromPeer(&block3)
